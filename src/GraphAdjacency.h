...
public:
  ...
  inline uint32_t getNumEdgeLabels() const {
    return (uint32_t) edge_label_pool.size();
  }

  // Get edges pointing towards the node by id
  inline const std::vector<Adjacency>& getAdjacencyIn(uint32_t vertex_id) const {
    return adjacency_in[vertex_id];
  }

  // Get edges pointing away from the node by id
  inline const std::vector<Adjacency>& getAdjacencyOut(uint32_t vertex_id) const {
  ...
  inline const std::string& getVertexLabel(uint32_t vertex_id) const {
    return vertex_labels[vertex_id];
  }

  // Get edge type/relation label/name by id
  inline const std::string& getEdgeLabel(uint32_t edge_label_id) const {
    return edge_label_pool[edge_label_id];
  }

  // Get node id by name/label
  inline uint32_t getVertexId(const std::string& label) const {
  ...
  inline uint32_t getEdgeLabelId(const std::string& label) const {
